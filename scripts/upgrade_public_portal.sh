...
# Atomically upgrade the three public deployment repositories:
#   1. mcp-observatory
#   2. mcp-native-guard
#   3. mcp-observatory-guard-portal
#
# The script clones exact remote commits into immutable release directories,
# builds/tests all three before downtime, serves a temporary 503 maintenance
# page, switches all current symlinks, restarts the portal, verifies health,
# and rolls back every switched repository if activation fails.
#
# Run on Lightsail as root:
...
#   OBSERVATORY_SKIP_ANALYZE_TEST=1   # useful on hosts without Docker
...
build_observatory() {
    log "configuring and building mcp-observatory"
    cmake --preset release -S "${new_observatory}"
    cmake --build "${new_observatory}/build/release" --parallel
    cmake --preset dev-debug -S "${new_observatory}"
    cmake --build "${new_observatory}/build/dev-debug" --parallel

    if [[ "${OBSERVATORY_SKIP_ANALYZE_TEST}" == "1" ]] && ! command -v docker >/dev/null 2>&1; then
        log "running mcp-observatory tests except Docker-dependent analyze CLI suite"
        ctest --test-dir "${new_observatory}/build/dev-debug" --output-on-failure -E '^mcpo_analyze_cli_tests$'
    else
        log "running complete mcp-observatory test suite"
        ctest --test-dir "${new_observatory}/build/dev-debug" --output-on-failure
    fi
}

build_native_guard() {
    log "configuring and building mcp-native-guard"
    cmake --preset release -S "${new_native_guard}"
    cmake --build "${new_native_guard}/build/release" --parallel
    cmake --preset dev-debug -S "${new_native_guard}"
    cmake --build "${new_native_guard}/build/dev-debug" --parallel
    ctest --test-dir "${new_native_guard}/build/dev-debug" --output-on-failure
}

build_portal() {
    log "compiling portal Python sources"
    python3 -m compileall -q "${new_portal}/mcp_portal" "${new_portal}/tests"
    log "running portal tests"
    (cd "${new_portal}" && python3 -m unittest discover -s tests -v)
}
...
<meta name="robots" content="noindex,nofollow,noarchive"><title>Upgrade in progress</title>
<style>:root{color-scheme:dark;font-family:system-ui,sans-serif}body{margin:0;min-height:100vh;display:grid;place-items:center;background:#111827;color:#f9fafb}main{width:min(42rem,calc(100% - 2rem));padding:2.5rem;border:1px solid #374151;border-radius:1rem;background:#1f2937}h1{font-size:clamp(1.8rem,5vw,3rem)}p{line-height:1.6;color:#d1d5db}.status{font-weight:700;color:#93c5fd}</style>
</head><body><main><p class="status">Scheduled maintenance</p><h1>Assurance platform upgrade in progress</h1>
<p>The catalog, assurance engine, and public read-only portal are being upgraded to their latest tested revisions.</p>
<p>No published catalog data is being modified by this deployment. Please retry in a few minutes.</p></main></body></html>"""
...
PY
    ...
    for _ in $(seq 1 30); do
        if [[ "$(curl -sS -o /dev/null -w '%{http_code}' "http://${MAINTENANCE_HOST}:${MAINTENANCE_PORT}/" || true)" == "503" ]]; then
            return 0
        fi
        sleep 0.2
    done
...
rollback() {
    ...
    wait_for_health && log "rollback restored previous deployment" || log "WARNING: rollback portal is not healthy"
}
...
main() {
    require_root
    for command in git cmake ctest ninja python3 systemctl curl flock runuser find sort cut awk; do
    ...
    systemctl cat "${PORTAL_SERVICE}" >/dev/null 2>&1 || fail "systemd service not found: ${PORTAL_SERVICE}"
