...
def dashboard(
    ...
    *,
    ...
    with self._connect() as connection:
        ...
        # Preserve the original longitudinal dashboard semantics:
        #
        #   Servers            = distinct identifiers seen across all history
        #   Immutable records  = all immutable server-version records retained
        #   Canonical artifacts = distinct canonical artifact digests retained
        #
        # These are intentionally NOT limited to the latest Registry snapshot.
        #
        # The latest-snapshot-specific counts continue to belong to the
        # "Latest published change summary" section.
        counted = connection.execute(
            """
            SELECT
                COUNT(DISTINCT server_identifier) AS servers,
                ...
                connection.execute(
                    ...
                    SELECT
                        ...
                        (
                            ...
                            ORDER BY p.position
                            LIMIT 1
                        ) AS package_transport
                    FROM snapshot_server_versions link
                    JOIN server_versions sv
                      ON sv.id=link.server_version_id
        ...
        if status.get("storage_v2_available"):
            ...
            analysis["recent"] = _rows(
                connection.execute(
                    ...
                    FROM recent_runs rr
                    JOIN server_versions sv
                      ON sv.id=rr.server_version_id
                    JOIN packages p
                      ON p.id=rr.package_id
                    LEFT JOIN analysis_v2_run_summaries s
                      ON s.analysis_run_id=rr.id
                    ORDER BY
                        rr.started_at COLLATE BINARY DESC,
                        rr.id DESC
                    """,
                    (recent_limit,),
                ).fetchall()
...
def analysis_coverage(self: Any) -> dict[str, Any]:
    with self._connect() as connection:
        ...
        coverage = (
            _coverage_row(connection)
            if "analysis_v2_coverage_summary" in tables
            else None
        )
        ...
        if coverage is not None:
            ...
            unreviewed = int(
                coverage["unreviewed_findings"] or 0
            )

            result.update(
                {
                    "eligible_package_records": int(
                        coverage["eligible_package_records"] or 0
                    ),
                    "analyzed_package_records": int(
                        coverage["completed_package_records"] or 0
                    ),
                    "failed_package_records": int(
                        coverage["failed_package_records"] or 0
                    ),
        ...
        result["storage_v2"] = {
            ...
            "history_available": (
                getattr(self, "_storage_v2_history_path", None)
                is not None
            ),
        }
